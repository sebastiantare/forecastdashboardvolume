...
@st.cache_data
def load_and_forecast(file, minus_3_months=False):
    ...
    if minus_3_months:
        # Edit this variable to change the
...
with tab1:
    ...
    if uploaded_file:
        ...
        unique_tasks.sort()
        selected_task = st.selectbox('**Select a Task**', unique_tasks)
        ...
        **Adjust Seasonality (for all Tasks)**
        ...
        edited_seasonal_data = st.data_editor(
            seasonal_data[seasonal_data.columns[1:]], width=800)
        adjusted_forecasted_data = forecasted_data.copy()

        if not adjusted_forecasted_data.empty:
            adjusted_forecasted_data.iloc[0, df.columns.get_loc(
                'Volume')] = original_data['Volume'].iloc[-1]

            for i in range(1, len(forecasted_data)):
                adjusted_forecasted_data.iloc[i, df.columns.get_loc('Volume')] *= 1 + (
                    edited_seasonal_data[edited_seasonal_data.columns.to_list()[i-1]] / 100)
        ...
        **Forecast Data (adjusted)**
        """
        st.dataframe(df_concat.reset_index(drop=True).style.set_precision(2), width=800)  # Adjust the width as needed
